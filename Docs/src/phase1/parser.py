...
from .config import DATA_PROCESSED_DIR, DATA_RAW_DIR, SCHEMES
from .utils import (
...
def extract_text_sections(soup: BeautifulSoup) -> list[dict]:
    ...
    if main is None:
        return [{"section": "General", "content": ""}]
    ...
    for el in main.descendants:
        if el.name in ["h1", "h2", "h3", "h4"]:
            ...
            current_section = section_text if section_text else "General"
            current_content = []
...
def parse_html(html_content: str) -> dict:
    ...
    try:
        if not html_content or not isinstance(html_content, str):
            logger.warning("[yellow]Empty or invalid HTML content[/]")
        ...
        logger.error(f"[red]Error parsing HTML:[/] {e}")
        import traceback
        logger.error(f"[red]Traceback:[/] {traceback.format_exc()}")
        return {"full_text": "", "sections": [], "structured_data": {}}
...
def parse_excel(excel_path: Path) -> dict:
    ...
    if not PANDAS_AVAILABLE:
        logger.error("[red]pandas not installed. Cannot parse Excel files.[/]")
        logger.error("[yellow]Install with: pip install pandas openpyxl[/]")
        return {"full_text": "", "sections": [], "structured_data": {}}
    try:
